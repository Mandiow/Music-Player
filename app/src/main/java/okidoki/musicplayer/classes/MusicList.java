...
public class MusicList extends Object implements Serializable, Cloneable, Runnable{
    ArrayList<Music> musicsList = new ArrayList<>();
    private String name;
    transient private FakeCache fakeCache = new FakeCache();
    ...
    public void WriteCache() {
        MusicList.queryPlaylists(MainActivity.mainActivity.getContentResolver());
        fakeCache.writeAlbum(MainActivity.mainActivity.getApplicationContext(), MainActivity.globalAlbumList);
        fakeCache.writeArtist(MainActivity.mainActivity.getApplicationContext(), MainActivity.globalArtistList);
        fakeCache.writeMusic(MainActivity.mainActivity.getApplicationContext(), MainActivity.globalMusicList);
    }
    public void ReadCache(){
        if (((MainActivity.globalMusicList = fakeCache.readMusic(MainActivity.mainActivity.getApplicationContext())) == null))
            Log.e("ERA", String.valueOf(MainActivity.globalAlbumList));
        MainActivity.globalAlbumList = fakeCache.readAlbum(MainActivity.mainActivity.getApplicationContext());
        MainActivity.globalAlbumList.FillBitmaps(MainActivity.globalAlbumList);
        MainActivity.globalArtistList = fakeCache.readArtist(MainActivity.mainActivity.getApplicationContext());
        MusicList.queryPlaylists(MainActivity.mainActivity.getContentResolver());
    ...
    @Override
    public void run() {
        //Toast.makeText(MainActivity.mainActivity.getApplicationContext(),"ENTREI",Toast.LENGTH_LONG).show();
        if(!FakeCache.Exists()) {
            searchForMusics();
            WriteCache();
        }
            ...
            ReadCache();
            PlayerFragment.musicPlayer.musicQueue = MainActivity.globalMusicList;
            MusicFragment.musicList = MainActivity.globalMusicList;
        }
        if (MainActivity.globalMusicList == null)
            Toast.makeText(MainActivity.mainActivity.getApplicationContext(),"BUCETA!",Toast.LENGTH_LONG).show();
        else
